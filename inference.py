...
def save_wifi(out_dir, data, pred, cond, batch, index=0):
    ...
    file_name_data = os.path.join(
        "./dataset/wifi/img_matric/data", f"out-{batch}-{index}.jpg"
    )
    file_name_pred = os.path.join(
        "./dataset/wifi/img_matric/pred", f"out-{batch}-{index}.jpg"
    )
...
def save_fmcw(out_dir, data, pred, cond, batch, index=0):
    ...
    file_name_data = os.path.join(
        "./dataset/fmcw/img_matric/data", f"out-{batch}-{index}.jpg"
    )
    file_name_pred = os.path.join(
        "./dataset/fmcw/img_matric/pred", f"out-{batch}-{index}.jpg"
    )
    ...
    with torch.no_grad():
        ...
        for features in tqdm(dataset, desc=f"Epoch {cur_batch // len(dataset)}"):
            ...
            if args.task_id in [0, 1]:
                ...
                pred_samples = [
                    torch.view_as_complex(sample)
                    for sample in torch.split(pred, 1, dim=0)
                ]  # [B, [1, N, S]]
                cond_samples = [
                    torch.view_as_complex(sample)
