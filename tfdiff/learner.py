...
class tfdiffLearner:
    def __init__(
        self, log_dir, model_dir, model, dataset, optimizer, params, *args, **kwargs
    ):
    ...
    def train(self, max_iter=None):
        ...
        while True:  # epoch
            for features in (
                tqdm(self.dataset, desc=f"Epoch {self.iter // len(self.dataset)}")
                if self.is_master
                else self.dataset
            ):
