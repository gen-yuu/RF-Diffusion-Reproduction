...
class WiFiDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"data": cur_data, "cond": cur_cond.squeeze(0)}
...
class FMCWDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cur_data = torch.from_numpy(cur_sample["feature"]).to(torch.complex64)
        cur_cond = torch.from_numpy(cur_sample["cond"].astype(np.int16)).to(
            torch.complex64
        )
        return {"data": cur_data, "cond": cur_cond.squeeze(0)}
...
class MIMODataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"data": torch.view_as_real(data), "cond": torch.view_as_real(cond)}
...
class Collator:
    ...
    def collate(self, minibatch):
        ...
        if task_id == 0:
            ...
            data = torch.stack(
                [record["data"] for record in minibatch if "data" in record]
            )
            cond = torch.stack(
                [record["cond"] for record in minibatch if "cond" in record]
            )
            return {
                "data": data,
